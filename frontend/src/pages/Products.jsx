...
const Products = () => {
  ...
  // Effect to handle URL param changes (search, category) and trigger fetch
  useEffect(() => {
    ...
    // Fetch products whenever URL params change (handles back/forward navigation)
    const promise = dispatch(fetchProducts({ isNewSearch: true }));

    return () => {
      promise.abort();
    };
  }, [search, category, dispatch]); // Depend on URL params

  // Infinite Scroll Handler
  const handleObserver = useCallback(
    (entries) => {
      const [target] = entries;
      if (target.isIntersecting && pagination.hasMore && !loading && !error) {
